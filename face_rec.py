...
hostname = 'redis-12155.c250.eu-central-1-1.ec2.cloud.redislabs.com'
portnumber = 12155
password = 'm7xt4p6BwiolMjlEmV9QeEuBMvSsYkQ1'
...
def compute_performance_metrics(dataframe, feature_column, test_vectors, true_labels, name_role=['Name', 'Role'], thresh=0.5):
    """
    Compute performance metrics for the ml_search_algorithm.
    """
    predicted_names = []
    predicted_roles = []
    true_names = []
    ...
    for test_vector in test_vectors:
        predicted_name, predicted_role = ml_search_algorithm(dataframe, feature_column, test_vector, name_role, thresh)
        predicted_names.append(predicted_name)
        predicted_roles.append(predicted_role)

    for true_name, true_role in true_labels:
        true_names.append(true_name)
        true_roles.append(true_role)

    # Compute performance metrics
    accuracy = accuracy_score(true_names, predicted_names)
    precision = precision_score(true_roles, predicted_roles, average='weighted')
    recall = recall_score(true_roles, predicted_roles, average='weighted')
